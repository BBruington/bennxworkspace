...
export default function DeityCard(props) {
  const {deity, alignment, title} = props

  return (
    <>
      <div className="overflow-hidden bg-white shadow sm:rounded-lg border border-gray-300 rounded-md cursor-pointer hover:bg-gray-100 ">
        <div className="px-4 py-5 sm:px-6 ">
          <h3 className="text-lg font-medium leading-6 text-gray-900">{deity}</h3>
          <p className="mt-1 max-w-2xl text-sm text-gray-500">{title}</p>
        </div>
        <div className="border-t border-gray-200">
          <dl>
            <div className="bg-gray-50 px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 ">
              <dt className="text-sm font-medium text-gray-500">{alignment}</dt>
              {/* <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">Elim</dd> */}
            </div>
